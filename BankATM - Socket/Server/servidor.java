...
public class servidor extends JFrame{
	...
	private void update_client(Socket Client) 
	{
		try {
			...
			pantalla.setText(pantalla.getText() +"\nActivity: "+Main_A.update_client(infoclient[0], infoclient[1], infoclient[2])+" Change info");
		} catch (Exception e) {
			pantalla.setText(e.getMessage());
		}
	}
	...
	private void add_client(Socket Client) 
	{
		try {
			...
			Client_aux = Client.getOutputStream();
			Client_envio = new DataOutputStream (Client_aux);
			Client_envio.writeUTF(""+Main_A.add_client(Long.parseLong(infoclient[0]), infoclient[1],infoclient[2], infoclient[3], Double.parseDouble(infoclient[4])));
