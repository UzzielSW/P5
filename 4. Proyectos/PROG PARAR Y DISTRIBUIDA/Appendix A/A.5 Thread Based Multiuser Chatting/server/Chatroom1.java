...
class Chatroom1 extends Dialog implements ActionListener {
	server s;
	TextArea ta1;
	...
	public Chatroom1(String s,server f){
		super(f,s,false);
	  ...
	  public void actionPerformed(ActionEvent ae)
		{
					 ...
					 if (ae.getSource().equals(bs))
					 	                 {
				try{
					list1.addItem("<Chatroom1>"+ta1.getText());
	for(int i = 0;i<c.size();i++){

		          t.bw=(BufferedWriter)cs.elementAt(i);
		          t.sendtoclients("<Chatroom1>"+ta1.getText());
                 // ta1.setText(" ");
			  }
			t.bw=temp;
