...
class ViewStudent extends JPanel implements ActionListener {
  ...
  public ViewStudent(RandomAccessFile raf) {
    ...
    p.add( jbtUpGrade = new JButton("UpGrade"));
    p.add(jtField = new JTextField(10));
    ...
    jbtUpGrade.addActionListener(this);
  }
      ...
      try {
        if ("First".equals(actionCommand)) {
          ...
          { System.out.println(currentPos);
           ...
           jtField.setText(toString().valueOf(primero));
           jbtFirst.setEnabled(false);
          ...
          if (lastPos > 0)
          	ultimo = primero;
          	jtField.setText(toString().valueOf(ultimo));
            retrieve(lastPos - 2*Student.RECORD_SIZE);
            System.out.println(currentPos);
            jbtLast.setEnabled(false);
            jbtNext.setEnabled(false);
            jbtPrevious.setEnabled(true);
        }else if ("UpGrade".equals(actionCommand)) {
        	currentPos = raf.getFilePointer();
        	if( currentPos > 0)
        	{
        	
        		
        	Student student2 = studentPanel.getStudent();
        		upGrade(currentPos);
  ...
  public void retrieve(long pos) {
   ...
   /** write a record at specified position */
  public void upGrade(long pos) {
    try {
      raf.seek(pos);
      student.writeStudent(raf);
      //student.readStudent(raf);
