 ...
 *                turns, but where one thread never
 *			receives a notify, and thus can never 
 *			exit.
 *
...
public class SafeTurns implements Runnable {
    private static Random rand = new Random();
    private TurnPrinter tp;
    private int myNum;

    public SafeTurns(TurnPrinter tp, int myNum) {
        this.tp = tp;
        this.myNum = myNum;
    }
    ...
    public static void main(String args[]) {
        ...
        (new Thread(new SafeTurns(tp, 0))).start();
        (new Thread(new SafeTurns(tp, 1))).start();
    }
}
